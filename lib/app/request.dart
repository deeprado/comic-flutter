...
import 'package:comic/utility/custom_cache_manager.dart';
...
class Request {
  ...
  static Future<dynamic> mock({String url, Map params}) async {
    var responseStr = await rootBundle.loadString('assets/mock/$url.json');
...
class CacheInterceptor extends InterceptorsWrapper {
  ...
  onRequest(RequestOptions options) async {
    FileInfo info = await ComicCacheManager().getFileFromCache(options.uri.toString());
    if (info != null) {
  ...
  onResponse(Response response) async {
    ...
    var data = response.data;
    await ComicCacheManager().putFile(response.request.uri.toString(), data);
    print("网络数据：" + data);
    return response;
  }
